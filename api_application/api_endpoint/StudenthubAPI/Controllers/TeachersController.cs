    ...
    public class TeachersController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetTeacherAssignmentsByUser(int userId)
        {
            try
            {
                ...
                try
                {
                    ...
                    {
                        cmd.CommandText = @"SELECT ta.TeacherAssignmentID AS Id, ta.TeacherID AS TeacherId, ta.LearningCentreID AS LearningCentreId,
                                                    ...
                                                    lc.LearningCentreID AS Learning_Centre_Id, lc.Name AS Learning_Centre_Name, lc.Latitude, lc.Longitude, lc.GeoRadiusMeters,lc.City,
                                                    p.ProgramID AS Program_Id, p.Name AS Program_Name,
                                                    ay.AcademicYearID AS AcademicYear_Id, ay.Name AS AcademicYear_Name, ta.ClusterID AS ClusterId, c.Name AS Cluster_Name
                                           FROM TeacherAssignments ta
                                           LEFT JOIN LearningCentres lc ON ta.LearningCentreID = lc.LearningCentreID
                        ...
                        using (var reader = await cmd.ExecuteReaderAsync())
                        {
                            while (await reader.ReadAsync())
                            {
                                ...
                                var learningCentreId = reader["LearningCentreId"] == DBNull.Value ? 0 : Convert.ToInt32(reader["LearningCentreId"]);
                                ...
                                var learningCentreName = reader["Learning_Centre_Name"] == DBNull.Value ? null : reader["Learning_Centre_Name"].ToString();
                                ...
                                var learningCentreCity = reader["City"] == DBNull.Value ? null : reader["City"].ToString();
                                ...
                                assignments.Add(new
                                {
                                    ...
                                    learning_centre_id = learningCentreId.ToString(),
                                    ...
                                    learning_centres = new { id = learningCentreId.ToString(), name = learningCentreName, latitude = latitude, longitude = longitude, geo_radius_meters = geoRadiusMeters, city = learningCentreCity },
        ...
        public async Task<IActionResult> CreateTeacher([FromBody] CreateTeacherBO createTeacherBO)
        {
            try
            {
                ...
                string tempPassword = Generate(12);
                string passwordHash = BCrypt.Net.BCrypt.HashPassword(tempPassword);

                await _dataContext.Database.ExecuteSqlRawAsync(
                    "EXEC sp_InsertTeacher @Name, @Gender, @DateOfBirth, @Email, @Phone, @Address, " +
                    "@City, @State, @Notes, @IDProofTypeID, @IDNumber, @Subjects, @StudentId, @PasswordHash, @TeacherID OUTPUT, @UserID OUTPUT, @Output OUTPUT",
                    ...
                    new SqlParameter("@Subjects", (object)createTeacherBO.Subjects ?? DBNull.Value),
                    new SqlParameter("@StudentId", (object)createTeacherBO.StudentId ?? DBNull.Value),
                ...
                {
                    // Send reset password email to the teacher
                    if (!string.IsNullOrWhiteSpace(createTeacherBO.Email))
                    {
                        ...
                        {
                            Console.WriteLine($"Failed to send reset password email: {emailEx.Message}");
                        }
                    }
        ...
        public async Task<IActionResult> UpdateTeacher(int id, [FromBody] UpdateTeacherBO updateTeacherBO)
        {
            try
            {
                ...
                await _dataContext.Database.ExecuteSqlRawAsync(
                    ...
                    "@City, @State, @Notes, @IDProofTypeID, @IDNumber, @Subjects, @StudentId, @Output OUTPUT",
                    new SqlParameter("@TeacherID", id),
                    ...
                    new SqlParameter("@Subjects", (object)updateTeacherBO.Subjects ?? DBNull.Value),
                    new SqlParameter("@StudentId", (object)updateTeacherBO.StudentId ?? DBNull.Value),
                    outputParameter);
        ...
        #region Ex-Student Lookup

        [Authorize]
        [HttpGet("ExStudent/{studentCode}")]
        public async Task<IActionResult> GetStudentByCode(string studentCode)
        {
            try
            {
                var student = _dataContext.Set<Student>()
                    .FromSqlRaw("EXEC spGetStudentByStudentCode @StudentCode",
